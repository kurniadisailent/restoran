          ...
          <div class="card-body">
            <div class="col-md-12">
              <div class="float-right">
                <form action="{{route('adminadminentritransaksi.cari')}}" method="GET">
              ...
              <ul class="nav nav-tabs mb-3">
	              <li class="nav-item">
                  <a class="nav-link {{ Request::is('admin/entritransaksi') ? 'active':'' }}" href="{{route('adminentritransaksi.index')}}"> Semua </a>
	              </li>
	              <li class="nav-item">
                <a class="nav-link {{ Request::is('admin/entritransaksi/status/selesai') ? 'active':'' }}" href="{{route('adminadminentritransaksi.status.selesai')}}"> Selesai </a>
	              </li>
	              <li class="nav-item">
                  <a class="nav-link {{ Request::is('admin/entritransaksi/status/belumbayar') ? 'active':'' }}" href="{{route('adminadminentritransaksi.status.belumbayar')}}"> Belum Bayar </a>
	              </li>
              </ul>
              <table class="table">
                  <thead>
                  ...
                  <?php $no = 1 ?>
                    @foreach($data as $row)
                      <tr>
                          ...
                          <td>
                            @if($row->status == 'SELESAI')
                              <a href="{{route('adminentritransaksi.view',['kodeorder'=>$row->kode_order])}}" target='_blank' class="btn btn-primary mr-3"><i class="fas fa-eye"></i></a>
                            @else
