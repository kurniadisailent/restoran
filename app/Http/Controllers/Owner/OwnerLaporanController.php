...
use App\Kasir;
use App\Laporan;
use App\Exports\LaporanExportMonth;
use App\Exports\LaporanExportWeek;
use App\Exports\LaporanExportYear;
...
class OwnerLaporanController extends Controller
{
    public function index()
    {
        ...
        $datatbl_laporan_satumingguterakhir = Laporan::whereBetween('tanggal',[$start_week, $end_week])->get();
        $datatbl_laporan_satubulanterakhir = Laporan::whereMonth('tanggal','=',Carbon::now()->subMonth()->month)->get();
        $datatbl_laporan_satutahunterakhir = Laporan::whereYear('tanggal', date('Y', strtotime('-1 year')))->get();
        // return dd(array_values($datachrt));
        // return dd($persentase_hari);
        return view('owner.laporan.index',[
            'data_laporan_minggu'=>$datatbl_laporan_satumingguterakhir,
            'data_laporan_bulan'=>$datatbl_laporan_satubulanterakhir,
            'data_laporan_tahun'=>$datatbl_laporan_satutahunterakhir,
    ...
    public function weekexport()
    {
        ...
        return Excel::download(new LaporanExportWeek, 'laporan_SatuMingguTerakhir_' . $thisdate . '.xlsx');
    }
    
    public function monthexport()
    {
        $thisdate = Carbon::now()->format('d-m-Y');
        return Excel::download(new LaporanExportMonth, 'laporan_SatuBulanTerakhir_' . $thisdate . '.xlsx');        
    }

    public function yearexport()
    {
        $thisdate = Carbon::now()->format('d-m-Y');
        return Excel::download(new LaporanExportYear, 'laporan_SatuTahunTerakhir_' . $thisdate . '.xlsx');        
    }
