...
class KasirEntriTransaksiController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('kasir.entriTransaksi.index',['data'=>$result]);
    }
    ...
    public function Cari(Request $request)
    {
        $result = Order::where('nama_pelanggan','like',"%{$request->keyword}%")->where('status','!=','SELESAI')
                        ...
                        ->orderBy('id_order','DESC')->paginate(10);
        return view('kasir.entriOrder.index',['data'=>$result]);
    }
