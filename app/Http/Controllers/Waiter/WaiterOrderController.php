...
class WaiterOrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $result = Menu::where('nama_menu','like',"%{$request->keyword}%")
                        ...
                        ->paginate(6);
        return view('waiter.order.index',['data'=>$result,'condition'=>$condition,'kat'=>$kategori]);
    }
    ...
    public function Order(Request $request)
    {
        ...
        if(empty($kode_order))
        {
            ...
            return  redirect()->route('waiterorder.cart',['data'=>$data,'id_petugas'=>$id_petugas]);
        }else{
            $id_order = DB::table('tbl_order')
                        ->select('id_order')
    ...
    public function OpenCart(Request $request, $id_petugas)
    {
        ...
        $jumlah_bayar = DB::table('tbl_detail_order')
                            ...
                            ->sum('total_bayar');
        $data = DetailOrder::where('kode_order','=',$kode_order)->get();
        return view('waiter.order.cart',['data'=>$data,'jumlah_bayar'=>$jumlah_bayar]);
    }
    ...
    public function DeleteCartItem($id_petugas, $kode_order, $id_detail_order)
    {
        ...
        $jumlah_bayar = DB::table('tbl_detail_order')
                            ...
                            ->sum('total_bayar');
        $data = DetailOrder::where('kode_order','=',$kode_order)->get();
        return redirect()->route('waiterorder.cart',['data'=>$data,'id_petugas'=>$id_petugas])->with('destroy','Berhasil dihapus!');
    }
