...
class WaiterEntriOrderController extends Controller
{
    public function index(Request $request)
    {
        $result = Order::where('status','=',"BELUM_BAYAR")->orWhere('status','=',"ORDER")->orderBy('tanggal','DESC')->paginate(10);
        return view('waiter.entriOrder.index',['data'=>$result]);
    }
    ...
    public function detail(Request $request,$kode_order)
    {
        ...
        $jumlah_bayar = DB::table('tbl_detail_order')
                            ...
                            ->sum('total_bayar');
        return view('waiter.entriorder.detail',['data'=>$result,'jumlah_bayar'=>$jumlah_bayar,'status'=>$status,'kode_order'=>$kode_order]);
    }
    ...
    public function ShowTambahMenu(Request $request, $kode_order)
    {
        ...
        $result = Menu::where('nama_menu','like',"%{$request->keyword}%")
                        ...
                        ->paginate(4);
        return view('waiter.entriOrder.menu',['data'=>$result,'condition'=>$condition,'kat'=>$kategori,'kode_order'=>$kode_order]);
    }
    ...
    public function TambahMenu(Request $request)
    {
        ...
        if(empty($check_detail_order))
        {
            ...
            return  redirect()->route('waiterwaiterentriorder.detail',['data'=>$result,'jumlah_bayar'=>$jumlah_bayar,'status'=>$status,'kode_order'=>$request->kode_order])->with('sukses',' Berhasil Menu Telah Di tambahkan');
    ...
    public function TambahJumlah(Request $request)
    {
        ...
        return redirect()->route('waiterorder.cart',['id_petugas'=>$request->id_petugas]);
    }
    ...
    public function ProsesOrder(Request $request)
    {
        ...
        for($i = 0; $i < $jumlah_data_dorder; $i++)
        {
            ...
            if (${'jumlah_pesan'.$i} > ${'jml_stk'.$i})
            {
                ...
                // return redirect()->route('waiterorder.cart',['id_petugas'=>$request->id_petugas])->with('stok',' Stok '. $namamenu->nama_menu . ' Tersisa ' . ${'jml_stk'.$i} . ' Silahkan Kurangi jumlah pemesanan');
            }
            // else{
        ...
        for($i = 0; $i < $jumlah_data_dorder; $i++)
        {
            ...
            if ($fotru == 1)
            {
                return redirect()->route('waiterorder.cart',['id_petugas'=>$request->id_petugas])->with('stok',' Stok '. $namamenu. ' ' . $stokmaskaan . ' Silahkan Kurangi jumlah pemesanan atau hubungi waiter');
            }
            else{
        ...
        $query2->update([
            'no_meja'=>$request->no_meja,
            'status'=>"BELUM_BAYAR",]);
        return redirect()->route('waiterentriorder.index')->with('berhasil','Order telah di proses !');
    }
    ...
    public function hapusdata($kodeorder)
    {
        ...
        $ddorder->delete();
        return redirect()->route('waiterentriorder.index')->with('berhasil','Order telah di hapus');
    }
}
